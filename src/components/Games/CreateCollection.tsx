...
export function useTrueWords(
  group: number,
  pageFromTextBook: number,
  wordsFromTextBook: WordData[],
  ...
  useEffect(() => {
    if (locationLaunch === 'menu') {
      ...
      if (!auth || !setAuth) {
        ...
        const dryWordsArray = wordsFromTextBook.filter(
          (word) => word.userWord?.difficulty !== 'learned'
        );
                ...
                prev
                  .concat(
                    res.data[0].paginatedResults
                      .filter(
                        (word) => word.page < page && word.group === group
                      )
                      .reverse()
                  )
                  .slice(0, count)
